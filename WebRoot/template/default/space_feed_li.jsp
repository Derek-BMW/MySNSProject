...
<li class="s_clear ${feed.magic_class }" id="feed_${feed.feedid }_li"
	...
	<div style="width: 100%; overflow: hidden;"${feed.style}>
		...
		<div class="feed_content">
			...
			<c:choose>
				<c:when test="${feed.thisapp>0 && !jch:jchEmpty(feed.body_data.flashvar)}">
				<div class="media">
					<div class="movie_app">
						<c:if test="${!jch:jchEmpty(feed.body_data.flashimg)}">
						...
						<img class="movie_pic" id="media_id_${feed.feedid}" src="${jch:jchEmpty(feed.body_data.flashimg)  ?  'image/vd.gif'  :  feed.body_data.flashimg}" alt="点击播放" onerror="this.onerror=null;this.src='image/vd.gif'" onclick="javascript:showFlash('${feed.body_data.host}', '${feed.body_data.flashvar}', this, '${feed.feedid}');" style="cursor: pointer;" />
					</div>
				</div>
				</c:when>
				<c:when
