...
<div class="tabs_header">
	<ul class="tabs">
		<li${actives.show}><a href="space.jsp?do=top"><span>竞价排行</span></a></li>
		<li${actives.mm}><a href="space.jsp?do=top&view=mm"><span>美女排行</span></a></li>
		<li${actives.gg}><a href="space.jsp?do=top&view=gg"><span>帅哥排行</span></a></li>
		<li${actives.experience}><a href="space.jsp?do=top&view=experience"><span>经验排行</span></a></li>
		<li${actives.credit}><a href="space.jsp?do=top&view=credit"><span>积分排行</span></a></li>
		<li${actives.friendnum}><a href="space.jsp?do=top&view=friendnum"><span>好友数排行</span></a></li>
		<li${actives.viewnum}><a href="space.jsp?do=top&view=viewnum"><span>访问量排行</span></a></li>
		<li${actives.online}><a href="space.jsp?do=top&view=online"><span>在线成员</span></a></li>
		<li${actives.updatetime}><a href="space.jsp?do=top&view=updatetime"><span>全部成员</span></a></li>
	</ul>
</div>
	...
	<c:if test="${now_pos>=0}">
		<div style="padding-bottom: 20px;">
			...
			<c:choose>
				...
				<c:otherwise>
					<c:choose>
						<c:when test="${param.view=='credit'}"><a href="cp.jsp?ac=credit">自己当前的积分：${space.credit}</a></c:when>
						<c:when test="${param.view=='friendnum'}"><a href="space.jsp?do=friend">自己当前的好友数：${space.friendnum}</a></c:when>
						<c:when test="${param.view=='experience'}">自己当前的经验数：${space.experience}</c:when>
						<c:otherwise><a href="space.jsp">自己当前的访问量：${space.viewnum}</a></c:otherwise>
					</c:choose>
			...
			<table width="100%">
				<tr>
					<td width="50%" valign="top">
						...
						<div class="content">
							<form method="post" action="cp.jsp?ac=top" onsubmit="return checkCredit('stakecredit');">
								<p>
					...
					<td width="50%" valign="top">
						...
						<div class="content">
							<form method="post" action="cp.jsp?ac=top" onsubmit="return checkCredit('showcredit');">
								<p>
