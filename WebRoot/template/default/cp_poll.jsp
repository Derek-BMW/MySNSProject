...
<div class="popupmenu_inner" id="${pid}">
	<form id="add_option_${pid}" name="add_option_${pid}" method="post" action="cp.jsp?ac=poll&op=addopt&pid=${pid}">
...
<div class="popupmenu_inner">
	<form method="post" action="cp.jsp?ac=poll&op=delete&pid=${pid}">
		<p>确定删除指定的投票吗？</p>
		<p class="btn_line">
			<input type="hidden" name="refer" value="${sGlobal.refer}" />
...
<a href="javascript:hideMenu();" class="float_del" title="关闭">关闭</a>
<div class="popupmenu_inner">
	<form method="post" action="cp.jsp?ac=poll&op=endreward&pid=${pid}">
...
<a href="javascript:hideMenu();" class="float_del" title="关闭">关闭</a>
<div class="popupmenu_inner">
<form method="post" action="cp.jsp?ac=poll&op=edithot&pid=${pid}">
	<p class="btn_line">
...
<a href="javascript:hideMenu();" class="float_del" title="关闭">关闭</a>
<div class="popupmenu_inner" id="${pid}">
	<form id="add_addreward_${pid}" name="add_addreward_${pid}" method="post" action="cp.jsp?ac=poll&op=addreward&pid=${pid}">
...
<form id="modify_expiration_${pid}" name="modify_expiration_${pid}" method="post" action="cp.jsp?ac=poll&op=modify&pid=${pid}">
<table>
	<tr>
		<td>
			<label for="expiration">请输入新的结束时间：</label><br />
			<input type="text" class="t_input" id="expiration" name="expiration" readonly value="${jch:sgmdate(pageContext.request,'yyyy-MM-dd',!jch:jchEmpty(poll.expiration) ? poll.expiration : sGlobal.timestamp+2592000,false) }" size="30" onclick="showcalendar(event, this, 0, '${jch:sgmdate(pageContext.request,'yyyy-MM-dd',sGlobal.timestamp,false)}')"/>
		</td>
	</tr>
	<tr>
		<td>
...
<form id="edit_summary_${pid}" name="edit_summary_${pid}" method="post" action="cp.jsp?ac=poll&op=summary&pid=${pid}">
<table>
	<tr>
		<td>
...
<c:when test="${op=='get'}">
	...
	<c:forEach items="${voteresult}" var="value">
		<li>
			...
			<c:choose>
			<c:when test="${jch:jchEmpty(value.username)}">
			匿名
			</c:when>
			<c:otherwise>
			...
			${jch:sgmdate(pageContext.request,'yyyy-MM-dd HH:mm:ss',value.dateline,true)} 投票给 ${value.option}
		</li>
	</c:forEach>
	</c:when>
	<c:otherwise>
	...
	<c:if test="${jch:jchEmpty(multi)}"><div class="page">${multi}</div><br/></c:if>
</c:when>
<c:when test="${op=='invite'}">

<form id="inviteform" name="inviteform" method="post" action="cp.jsp?ac=poll&op=invite&pid=${poll.pid}&uid=${param.uid}&grade=${param.grade}&group=${param.group}&page=${param.page}&start=${param.start}">

<h2 class="title"><img src="image/app/poll.gif" />投票</h2>
<div class="tabs_header">
	<ul class="tabs">
		<li><a href="cp.jsp?ac=poll"><span>发起新投票</span></a></li>
		<li class="active"><a href="cp.jsp?ac=poll&op=invite&pid=${poll.pid}"><span>邀请好友</span></a></li>
		<li><a href="space.jsp?uid=${poll.uid}&do=poll&pid=${poll.pid}"><span>返回投票</span></a></li>
	</ul>
</div>
<div id="content" style="width: 640px;">
	<div class="h_status">
		您可以邀请下列好友来参与<a href="space.jsp?uid=${poll.uid}&do=poll&pid=${poll.pid}">《${poll.subject}》</a>投票
	</div>
	...
	<div class="h_status">
		...
		<c:forEach items="${list}" var="value">
			<li><div class="avatar48"><a href="space.jsp?uid=${value.fuid}" title="${sNames[value.fuid]}">${jch:avatar1(value.fuid,sGlobal,sConfig)}</a></div>
				...
				<p><c:choose><c:when test="${jch:jchEmpty(invitearr[value.fuid])}"><input type="checkbox" name="ids" value="${value.fuid}">选定</c:when><c:otherwise>已邀请</c:otherwise></c:choose></p>
			</li>
		</c:forEach>
		</ul>
...
<div id="sidebar" style="width: 150px;">
	<div class="cat">
		...
		<ul class="post_list line_list">
			<li<c:if test="${param.group==-1}"> class="current"</c:if>><a href="cp.jsp?ac=poll&pid=${poll.pid}&op=invite&group=-1">全部好友</a></li>
			<c:forEach items="${groups}" var="group">
			<li<c:if test="${param.group==group.key}"> class="current"</c:if>><a href="cp.jsp?ac=poll&pid=${poll.pid}&op=invite&group=${group.key }">${group.value}</a></li>
			</c:forEach>
		</ul>
	</div>
</div>
...
<c:otherwise>
	...
	<div class="tabs_header">
		<ul class="tabs">
			<li class="active"><a href="cp.jsp?ac=poll"><span>发起新投票</span></a></li>
			<li><a href="space.jsp?do=poll&view=me"><span>返回我的投票</span></a></li>
		</ul>
	</div>
</c:otherwise>
</c:choose>
...
<div class="c_form">
	...
	<form id="addnewpoll" name="addnewpoll" method="post" action="cp.jsp?ac=poll">
		<table cellspacing="4" cellpadding="4" width="100%" class="infotable">
			...
			<tr>
				...
				<td>
					...
					<input type="text" class="t_input" size="16" id="expiration" readonly name="expiration" value="" onclick="showcalendar(event, this, 0, '${jch:sgmdate(pageContext.request,'yyyy-MM-dd',sGlobal.timestamp,false)}')" />
				</td>
			</tr>
			<tr>
				<th>投票限制</th>
			...
			<tr>
				<th>动态选项</th>
				<td>
					<input type="checkbox" name="makefeed" id="makefeed" value="1"${ckPrivacyBypoll  ?  ' checked' : ''}> 产生动态 (<a href="cp.jsp?ac=privacy#feed" target="_blank">更改默认设置</a>)
				</td>
			</tr>
	...
	<script type="text/javascript" charset="${jchConfig.charset}">
		...
		function validate(obj) {
			...
			var nowDate = parsedate("${jch:sgmdate(pageContext.request,'yyyy-MM-dd',sGlobal.timestamp,false)}");
