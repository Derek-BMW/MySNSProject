...
<c:if test="${jch:jchEmpty(ajax_edit)}"><li id="share_${share.sid}_li"></c:if>
	...
	<div class="media">
	<div class="movie_app">
	<c:if test="${!jch:jchEmpty(share.body_data.flashimg)}">
	...
	<img class="movie_pic" id="media_id_${share.sid}" src="${jch:jchEmpty(share.body_data.flashimg)  ?  'image/vd.gif'  :  share.body_data.flashimg}" alt="点击播放" onerror="this.onerror=null;this.src='image/vd.gif'" onclick="javascript:showFlash('${share.body_data.host}', '${share.body_data.flashvar}', this, '${share.sid}');" style="cursor:pointer;" />
	</div>
	</div>
	</c:when>
