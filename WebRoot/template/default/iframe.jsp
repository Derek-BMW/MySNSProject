...
<html>
 <head>
  ...
  <title><c:if test="${not empty space}">${sNames[space.uid]} - </c:if>${sConfig.sitename} - Powered by JavaCenter Home</title>
 ...
 <body scroll="no">
  ...
  <div id="divshare" style="position: absolute; left: 50%; top: 100px; width: 450px; w\idth: 434px; margin-left: -225px; border: 8px solid #999; background: #FFF; display: none; padding: 20px;">
   <form method="post" id="shareform" name="shareform" action="cp.jsp?ac=share&type=link">
    <table>
     <tr>
      <td>
     ...
     <tr>
      <td>
       <input type="hidden" name="refer" value="space.jsp?do=share">
    ...
    <ul class="box" style="text-align: left;">
     <li id="share_li">
      <div class="title"><a href="space.jsp?uid=${space.uid}">${sNames[space.uid]}</a> 分享了一个网址 <span class="gray">${timestamp}</span></div>
      <div class="feed">
