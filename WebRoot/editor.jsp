...
<html>
 <c:choose><c:when test="${jch:jchEmpty(param.op)}">
  <head>
  ...
  <body style="overflow-y: hidden">
      ...
      <tr><td height="31" style="padding-left: 3px">
       ...
       <div class="toobar">
        <div class="icons tble">
         ...
         <c:if test="${!jch:jchEmpty(param.doodle)}"><a href="javascript:;" class="icoDoodle" id="doodleBox" onClick="doodleBox(event, this.id);return false;" title="涂鸦"></a></c:if>
         <a href="javascript:;" class="icoRenew" onClick="renewContent();return false;" title="恢复内容"></a>
         <c:choose>
          <c:when test="${!jch:jchEmpty(param.allowhtml)}"><input type="checkbox" value="1" name="switchMode" id="switchMode" style="float: left; margin-top: 6px !important; margin-top: 2px" onClick="setMode(this.checked)" onMouseOver="fSetModeTip(this)" onMouseOut="fHideTip()"></c:when>
     ...
     <table cellpadding="0" cellspacing="0" style="background-color: #999999; height: 100%; width: 100%; overflow: hidden">
      <tr><td>
       ...
       <!--
        function blank_load() {
         var inihtml = '';
         var obj = parent.document.getElementById('jchome-ttHtmlEditor');
         if(obj) {
          inihtml = obj.value;
         }
        ...
        if(document.all){
         document.write('<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0" id="divEditor"><tr><td style=""><IFRAME class="HtmlEditor" ID="HtmlEditor" name="HtmlEditor" style="height:100%;width:100%" frameBorder="0" marginHeight=0 marginWidth=0 src="editor.jsp?op=blank" onload="blank_load();"></IFRAME></td></tr></table>');
        }else{
         document.write('<table width="100%" height="100%" border="0" cellspacing="0" cellpadding="0" id="divEditor"><tr><td style="background-color:#ffffff"><IFRAME class="HtmlEditor" ID="HtmlEditor" name="HtmlEditor" style="height:100%;width:100%;margin-left:1px;margin-bottom:1px;" frameBorder="0" marginHeight=0 marginWidth=0 src="editor.jsp?op=blank" onload="blank_load();"></IFRAME></td></tr></table>');
        }
       //-->
       </script>
   ...
   <input type="hidden" name="jchome-editstatus" id="jchome-editstatus" value="html">
  </body>
 </c:when><c:otherwise>
  <head>
   <meta http-equiv="Content-Type" content="text/html;charset=${jchConfig.charset}" />
