...
<div class="mainarea">
 <div class="maininner">
  <div class="tabs_header">
   ...
   <ul class="tabs">
    <li id="title_0" class="active"><a href="admincp.jsp?ac=config#base"><span onclick="configShow('0')">基本设置</span></a></li>
    <li id="title_1"><a href="admincp.jsp?ac=config#register"><span onclick="configShow('1')">注册与显示</span></a></li>
    <li id="title_2"><a href="admincp.jsp?ac=config#jch"><span onclick="configShow('2')">系统功能</span></a></li>
    <li id="title_3"><a href="admincp.jsp?ac=config#name"><span onclick="configShow('3')">实名认证设置</span></a></li>
    <li id="title_4"><a href="admincp.jsp?ac=config#video"><span onclick="configShow('4')">视频认证设置</span></a></li>
    <li id="title_5"><a href="admincp.jsp?ac=config#upload"><span onclick="configShow('5')">上传图片设置</span></a></li>
    <li id="title_6"><a href="admincp.jsp?ac=config#remote"><span onclick="configShow('6')">远程上传设置</span></a></li>
    <li id="title_7"><a href="admincp.jsp?ac=config#email"><span onclick="configShow('7')">邮件设置</span></a></li>
   </ul>
  </div>
  <form method="post" action="admincp.jsp?ac=config">
   ...
   <div class="bdrcontent" id="config_0">
    ...
    <table class="formtable">
     ...
     <tr>
      <th>站点访问URL地址</th>
      <td><input type="text" name="config[siteallurl]" value="${configs.siteallurl}" size="50"><br>开启二级域名时需要使用，例如：http://j.jsprun.net/ ，末尾要加“/”</td>
     </tr>
     <tr>
      <th>站点模板</th>
      <td>
     ...
     <tr>
      ...
      <td>
       ...
       <br>JavaCenter用户头像分为两套体系(默认为体系1)，您可以选择在本系统内使用的头像体系，而在其他应用（比如JspRun!论坛）中使用相同或者不同的头像体系。<br>注意：修改本选项后，用户需要上传与体系对应的新头像。
      </td>
     </tr>
     <tr>
      <th>JavaCenter的物理路径</th>
      <td>
       <input type="text" name="config[jc_dir]" value="${configs.jc_dir}" size="40">
       <br>默认为空。如果本系统可以读取JavaCenter的程序目录，推荐填写该路径值。
       <br>请输入以 “./” “../”开头的相对路径，或者绝对路径，末尾加 "/"，例如填写：../javacenter/。
      </td>
     </tr>
     <!-- 屏蔽应用
     <tr>
    ...
    <table class="formtable">
     ...
     <tr>
      ...
      <td>
       ...
       <br>请输入要折叠显示的动态类型（icon），多个类型之间用半角逗号 "," 隔开。 <br>将某些动态折叠后，可以适当减少首页的动态噪音。<br>动态类型的获取方法：
       <br>在查看好友动态的时候，点击每条动态前面的小图标（例如 <img src="image/icon/blog.gif" align="absmiddle">），
       <br>浏览器的网址会变为类似如下的链接：<br>http://xxx.com/space.jsp?uid=&do=feed&view=we&appid=3&<b>icon=blog</b>，
     ...
     <tr>
      <th>成员排行榜允许分页浏览</th>
      <td>
       <input type="radio" name="config[networkpage]" value="1" ${configs.networkpage == 1 ? "checked" : ""}>是
       <input type="radio" name="config[networkpage]" value="0" ${configs.networkpage != 1 ? "checked" : ""}>否
    ...
    <table class="formtable">
     ...
     <tr>
      ...
      <td>
       ...
       <br>注意: 本功能会严重加重服务器负担。此功能需要web服务器为Tomcat6.x，并需要配置Tomcat的server.xml，修改Connector节点的protocol属性值为"org.apache.coyote.http11.Http11NioProtocol"。
     ...
     <tr>
      <th>JavaCenter应用标签相关信息</th>
      <td>
       <table>
        <tr>
         <td width="150">是否启用</td>
         <td>
          <input type="radio" name="config[jc_tagrelated]" value="1" ${configs.jc_tagrelated == 1 ? "checked" : ""}>是
          <input type="radio" name="config[jc_tagrelated]" value="0" ${configs.jc_tagrelated != 1 ? "checked" : ""}>否
          <br>开启本功能，则系统会通过JavaCenter，获取到站内其他应用的标签相关信息<br>否则，只显示本站内的标签相关日志
         </td>
        </tr>
        <tr>
         <td>缓存更新间隔</td>
         <td>
          <input type="text" name="config[jc_tagrelatedtime]" value="${configs.jc_tagrelatedtime}" size="10" maxlength="10"> (单位:秒, 默认为: 86400 即，24小时)
