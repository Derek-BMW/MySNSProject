...
<div class="maininner">
	<div class="tabs_header">
		<ul class="tabs">
			<li ${active}><a href="admincp.jsp?ac=block"><span>浏览全部模块</span></a></li>
			<li class="null"><a href="admincp.jsp?ac=block&op=add">添加新模块</a></li>
		</ul>
	</div>
<c:choose>
<c:when test="${empty param.op}">
	<div class="bdrcontent">
		<p>数据调用，会将站内的数据，通过你编写的查询SQL语句，进行查询并读取出来，生成一段调用代码。你将调用代码(模板内嵌，或者JS调用都可以)放置到站点页面上便可以将相应的查询结果展示给访客了，从而可以实现站内任意数据的显示调用。</p>
	</div>
	<br />
	<div class="bdrcontent">
...
<c:when test="${param.op == 'add' || param.op == 'blocksql'}">
	...
	<div class="bdrcontent">
		...
		<tr><th style="width:10em;">数据调用SQL</th><td><textarea id="blocksql" name="blocksql" style="width:98%;" rows="6">${block.blocksql}</textarea>
			...
			<br />1. 查询读取最新的日志 (假如表名前缀为默认的 jchome_)
			<br />SELECT * FROM jchome_blog ORDER BY dateline DESC
			<br />2. 查询读取论坛的最新帖子 (假如论坛安装在jsprun数据库，表名前缀为 jrun_)
			<br />SELECT * FROM jsprun.jrun_threads ORDER BY dateline DESC
			</td></tr>
		</table>
	</div>
	...
	var sqls = [];
	var tablePre = '${jchConfig.tablePre}';
	//////JSTL脚本//////
...
<c:when test="${param.op == 'code'}">
	...
	<tr><th>数据显示HTML代码</th><td>
		...
		<br />获取到的数据存放在数组 SBLOCK[变量名] 中(将“变量名”替换为你在上面设定的变量名)，可以使用 forEach 语法对该数组变量进行循环展示。请参考程序包中《数据调用》文档。
	</td></tr>
