/*
 * @(#)$Id: script_upload.js,v 1.3 2011/01/12 14:01:27 cvsroot Exp $
 * 
...
function insertAttachimgTag(id) {
	if(insertType == 0) {
		...
		var imgCache = new Image();
		imgCache.src = picPath;
		edit_insert('<img id="_jchome_localimg_' + id + '" style="filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\''+picPath+'\')" src=\'image/none.gif\'/>');
	}else if($('img_' + id)){
		var picPath =$('img_' + id).src;
		edit_insert('<img id="_jchome_localimg_' + id + '" src="'+picPath+'"/>');
	}else{
